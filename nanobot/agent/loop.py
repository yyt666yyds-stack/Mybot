...
from nanobot.utils.helpers import extract_thinking_text, image_placeholder_text
...
class _LoopHook(AgentHook):
    ...
    async def on_stream(self, context: AgentHookContext, delta: str) -> None:
        ...
        prev_thinking = extract_thinking_text(self._stream_buf)
        self._stream_buf += delta
        new_clean = strip_think(self._stream_buf)
        new_thinking = extract_thinking_text(self._stream_buf)

        content_inc = new_clean[len(prev_clean):]
        thinking_inc = new_thinking[len(prev_thinking):]

        if thinking_inc and self._on_thinking:
            await self._on_thinking(thinking_inc)
        if content_inc and self._on_stream:
            await self._on_stream(content_inc)
    ...
    async def on_thinking(self, context: AgentHookContext, delta: str) -> None:
        if self._on_thinking:
            await self._on_thinking(delta)
...
class AgentLoop:
    ...
    def __init__(
        ...
        from nanobot.config.schema import ProvidersConfig as _ProvidersConfig
        ...
        self.providers_config = providers_config or _ProvidersConfig()
    ...
    def _register_default_tools(self) -> None:
        ...
        self._vision_tool: VisionTool | None = None
        if self.tools_config.vision.enabled:
            self._vision_tool = VisionTool(
                config=self.tools_config.vision,
                providers=self.providers_config,
            )
            self.tools.register(self._vision_tool)
    ...
    async def _dispatch(self, msg: InboundMessage) -> None:
        ...
        try:
            async with lock, gate:
                try:
                    on_stream = on_stream_end = on_thinking = None
    ...
    async def _auto_describe_images(
        self, initial_messages: list[dict[str, Any]], media: list[str]
    ) -> list[dict[str, Any]]:
        """Auto-describe images before the main LLM turn.

        Replaces image_url blocks in the user message with text descriptions,
        so the main LLM sees the descriptions directly in the user context.
        """
        if not self._vision_tool:
            logger.debug("[vision] vision_tool is None, skipping auto-describe")
            return initial_messages
        if not self.tools_config.vision.auto_describe:
            logger.debug("[vision] auto_describe disabled in config, skipping")
            return initial_messages
        if not media:
            return initial_messages

        logger.info("[vision] auto-describe: {} images in media", len(media))

        limit = self.tools_config.vision.max_images_per_turn
        if limit <= 0:
            return initial_messages

        to_describe = media[:limit]
        skipped = len(media) - len(to_describe)

        async def _describe_one(idx: int, img_path: str) -> str | None:
            try:
                result = await self._vision_tool.execute(path=img_path)
        ...
        descriptions: list[str | None] = await asyncio.tasks.gather(
            *[_describe_one(i, p) for i, p in enumerate(to_describe)]
        )

        ok = sum(1 for d in descriptions if d is not None)
        logger.info("[vision] described {}/{} images successfully", ok, len(to_describe))

        # Replace image_url blocks in the user message with text descriptions.
        result = list(initial_messages)
        user_msg = result[-1]
        ...
        for block in content:
            if block.get("type") == "image_url":
                if desc_idx < len(descriptions):
                    ...
                    if desc:
                        label = f"[图片 {desc_idx + 1}/{len(to_describe)} 描述]"
                        new_content.append({"type": "text", "text": f"{label}: {desc}"})
                    else:
                        path = to_describe[desc_idx]
                        placeholder = image_placeholder_text(path, empty="[image omitted]")
                        new_content.append({
                            "type": "text",
                            "text": f"[{placeholder} 图片描述生成失败]",
                        })
                    desc_idx += 1
                else:
                    # Image beyond limit, keep placeholder
        ...
        if skipped > 0:
            new_content.append({
                "type": "text",
                "text": f"[剩余 {skipped} 张图片超出单轮上限，已跳过]",
            })
