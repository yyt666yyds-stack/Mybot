...
def extract_thinking_text(text: str) -> str:
    """Extract text inside <think>/<thought> tags, handling unclosed streaming blocks.

    Companion to ``strip_think``: while ``strip_think`` removes thinking content,
    this function extracts it for separate streaming to the frontend.
    """
    parts: list[str] = []
    for tag in ("think", "thought"):
        ...
        if last_open > last_close:
            parts.append(text[last_open + len(f"<{tag}>"):])
    return "".join(parts)
